...
for filename in split_files:
    ...
    output_path = os.path.join(output_folder, f"HS-brexit_{split_name}_softlabel.csv")
    ...
    with open(output_path, "w", encoding="utf-8", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["item_id", "text", "p_0", "p_1", "hard_label"])

        for item_id, item in data.items():
            soft_label_dict = item["soft_label"]

            # enforce class order
            p_0 = float(soft_label_dict["0"])
            p_1 = float(soft_label_dict["1"])
