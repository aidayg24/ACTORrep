"""
train_actor_paper_group_weighted_round3.py

Purpose
-------
Round-3 weighted ACTOR training for the paper-style active learning replication.

This script:
1. Loads the Round-3 weighted labeled pool produced by:
...
5. Saves a clean state_dict for the next acquisition round.

Design choice
-------------
This script intentionally follows the same structure/paradigm as the working
Round-1 and Round-2 weighted training scripts, so the pipeline stays consistent.

Important
---------
Do NOT switch to majority-vote dev/test files here.
ACTOR training must use annotation-level dev/test files (the ones with 1008 rows).
"""
...
TRAIN_PATH = "../processed_data/hsbrexit_paper_group_weighted_labeled_pool_round3.csv"

# IMPORTANT:
# These must be the same annotation-level ACTOR dev/test files
# you already used successfully in previous ACTOR training runs.
DEV_PATH = "../processed_data/hsbrexit_dev_annotations.csv"
TEST_PATH = "../processed_data/hsbrexit_test_annotations.csv"

OUTPUT_DIR = "../outputs/actor_paper_group_weighted_round3"
...
print(f"Paper group weighted round-3 labeled pool size: {len(train_df)}")
print(f"Dev size: {len(dev_df)}")
print(f"Test size: {len(test_df)}")
print()

print("Round-3 pool label distribution:")
...
# Training args
# ============================================================
# save_strategy="no" is crucial because Trainer checkpoint saving
# crashes on shared tensors (class_weights/loss_fn.weight).
...
print()
print("Starting training on paper group weighted round-3 pool...")
