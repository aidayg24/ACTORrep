"""
Train the supervised ACTOR model on the initial labeled pool only.

What this script does
---------------------
This is the first real step of the active learning pipeline.

We already created:
- an initial labeled pool
- an unlabeled pool

Now we train ACTOR only on the initial labeled pool, while still evaluating on
the original dev/test annotation-level sets.

Why this matters
----------------
This gives us the round-0 active learning baseline.
After this works, we will add:
1. random acquisition from the unlabeled pool
2. pool update
3. retraining for the next round
"""
...
# 1. Load datasets
# ---------------------------
train_path = "../processed_data/hsbrexit_initial_labeled_pool.csv"
...
print("Initial labeled pool size:", len(train_df))
print("Dev size:", len(dev_df))
print("Test size:", len(test_df))

print("\nInitial pool label distribution:")
print(train_df["label"].value_counts())

# ---------------------------
# 2. Create annotator mapping
# Use all annotators appearing in the initial pool
# ---------------------------
...
training_args = TrainingArguments(
    output_dir="../outputs/actor_initial_pool",
    eval_strategy="epoch",
    save_strategy="epoch",
...
# 11. Train
# ---------------------------
print("\nStarting training on initial labeled pool...")
trainer.train()
